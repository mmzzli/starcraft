<template>
  <section class="container">
    ...
    <ul
      class="list"
      :class="{
        'left-1': pageList.length % 3 == 1,
        'left-2': pageList.length % 3 == 2
      }"
    >
      <li
    ...
    >
      <div class="popupbox1 rich_plan_popup">
        ...
        <div class="tips_content">
          When A Friend Purchases SCT, The Inviter Will Receive A One-Timreward
          Of 1%
        </div>
...
const pageList = computed(() => {
  ...
  console.log(isPurchased.value, "===isPurchased==");
  if (isDrawed.value == 2) {
    list.unshift({
      path: "/Mining",
      img: img15,
...
const getIsDrawed = async () => {
  ...
  try {
    ...
    if (Number(res) > 0) {
      ...
      const tokens = await scp().tokensOfOwnerBySize(
        walletAccount.value,
        0,
        1000
      );
